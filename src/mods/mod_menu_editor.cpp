...
ModMenuEditor::ModMenuEditor(Context &ctx)
    : ModBase(ctx, MOD_FILENAME)
      ...
      , m_current_field(0) {
    // Setup Modules
    m_setup_functions.emplace_back([this] { setupMenuEditor(); });
    m_setup_functions.emplace_back([this] { setupMenuOptionEditor(); });
    m_setup_functions.emplace_back([this] { setupMenuEditFields(); });
    m_setup_functions.emplace_back([this] { setupMenuOptionEditFields(); });

    // Input or Method Modules that handle incoming input per state.
    m_mod_functions.emplace_back(
        [this](const std::string &s) { return menuEditorInput(s); });
    m_mod_functions.emplace_back(
        [this](const std::string &s) { return menuEditorPausedInput(s); });
    m_mod_functions.emplace_back(
        [this](const std::string &s) { return menuEditorMenuNameInput(s); });

    // Menu Option Input Commands.
    m_mod_functions.emplace_back(
        [this](const std::string &s) { return menuEditorOptionInput(s); });
    m_mod_functions.emplace_back(
        [this](const std::string &s) { return menuEditorMenuOptionInput(s); });

    // Menu Field Input Commands
    m_mod_functions.emplace_back(
        [this](const std::string &s) { return menuEditorMenuFieldInput(s); });
    m_mod_functions.emplace_back(
        [this](const std::string &s) { return menuEditorMenuFieldHandler(s); });

    // Menu Option Field Input Commands.
    m_mod_functions.emplace_back(
        [this](const std::string &s) { return menuEditorMenuOptionFieldInput(s); });
    m_mod_functions.emplace_back(
        [this](const std::string &s) { return menuEditorMenuOptionFieldHandler(s); });

    // Display Pause, waits for a key, then returns (Used in View Generic Menu)
    m_mod_functions.emplace_back(
        [this](const std::string &s) { return menuEditorDisplayPause(s); });
...
void ModMenuEditor::createTextPrompts() {
    ...
    value[PROMPT_HEADER] = std::make_pair("Menu Editor Header",
                                          "|CS|CR|03--- |15[|03Oblivion/2 XRM |07// |11Menu Editor|15] |03--- |CR");
    value[PROMPT_OPTION_HEADER] = std::make_pair("Menu Options Editor Header |OT Menu ID",
                                                 "|CS|CR|03--- |15[|03Oblivion/2 XRM |07// |11Menu Option Editor|15] |03--- |11Menu ID : |15|OT |CR");
    value[PROMPT_MENU_EDIT_HEADER] = std::make_pair("Menu Fields Editor Header |OT Menu ID",
                                                    "|CS|CR|03--- |15[|03Oblivion/2 XRM |07// |11Menu Editor|15] |03--- |11Menu ID : |15|OT |CR");
    value[PROMPT_PAUSE] = std::make_pair("Pause Prompt",
                                         "|CR |03- |15Hit any key to continue or (|03a|15)bort listing |03-|15 |CR");
    value[PROMPT_INPUT_TEXT] = std::make_pair("Menu Edit Prompt",
                                              "|CR|03A|15/dd Menu |03E|15/dit Menu |03D|15/elete Menu |03C|15/opy Menu |03Q|15/uit : ");
    value[PROMPT_OPTION_INPUT_TEXT] = std::make_pair("Menu Option Edit Prompt",
                                                     "|CR|03A|15/dd |03E|15/dit |03D|15/elete |03C|15/opy |03M|15/ove |03T|15/oggle |03Q|15/uit : ");
    ...
    value[PROMPT_INVALID_MENU_NOT_EXISTS] = std::make_pair("Invalid Menu Doesn't Exist",
                                                           "|CR|04Invalid, Menu doesn't exist.|CR");

    // Menu Field Edit
    value[PROMPT_MENU_FIELD_INPUT_TEXT] = std::make_pair("Menu Field Edit Prompt",
                                                         "|CR|15Menu Editor C|07om|08mand |15: |07");

    // NOTE, added |PD will display the prompt description as HELP text to the user
    // Useful when editing fields - Specific to XRM.
    value[PROMPT_MENU_FIELD_TITLE] = std::make_pair("Menu Title (Shown in ANSI screens with 'MT' MCI code)",
                                                    "|CR|03%   |15|PD|CR|11!   |03(|11A|03) |15Menu Title         : ");
    value[PROMPT_MENU_FIELD_PASSWORD] = std::make_pair(
        "Menu Password (Clear Text right now, will be encrypted lateron)",
        "|CR|03%   |15|PD|CR|11!   |03(|11B|03) |15Password           : ");
    value[PROMPT_MENU_FIELD_FALLBACK] = std::make_pair("Fallback Menu (Example 'MAIN')",
                                                       "|CR|03%   |15|PD|CR|11!   |03(|11C|03) |15Fallback Menu      : ");
    value[PROMPT_MENU_FIELD_HELP_ID] = std::make_pair(
        "ANSI Help File (Example 'MATRIX' Without extension) Overrides Generic Menu",
        "|CR|03%   |15|PD|CR|11!   |03(|11D|03) |15Help ID            : ");
    value[PROMPT_MENU_FIELD_NAME] = std::make_pair("Menu Name Displayed in Prompts",
                                                   "|CR|03%   |15|PD|CR|11!   |03(|11E|03) |15Name in Prompt     : ");
    value[PROMPT_MENU_FIELD_PULLDOWN] = std::make_pair("Pulldown ANSI with extension (Example 'MATRIX.ANS')",
                                                       "|CR|03%   |15|PD|CR|11!   |03(|11F|03) |15Pulldown File      : ");

    // Menu Option Prompts
    value[PROMPT_INVALID_OPTION_NOT_EXISTS] = std::make_pair("Invalid Option Doesn't Exist",
                                                             "|CR|04Invalid, Option doesn't exist.|CR");
    value[PROMPT_MENU_OPTION_ADD] = std::make_pair("Menu Option Add Before Which Command",
                                                   "|CR|11CREATE|15 and insert before which option : ");
    value[PROMPT_MENU_OPTION_DELETE] = std::make_pair("Menu Option Delete At Index",
                                                      "|CR|11DELETE|15 which option number : ");
    value[PROMPT_MENU_OPTION_CHANGE] = std::make_pair("Menu Option To Change",
                                                      "|CR|15Enter option number to |11EDIT|15 : ");
    value[PROMPT_MENU_OPTION_COPY_FROM] = std::make_pair("Menu Option To Copy From",
                                                         "|CR|15Enter option number to |11COPY|15 : ");
    value[PROMPT_MENU_OPTION_COPY_TO] = std::make_pair("Menu Option To Copy To",
                                                       "|11SAVE|15 as and insert before which option : ");
    value[PROMPT_MENU_OPTION_MOVE_FROM] = std::make_pair("Menu Option To Copy From",
                                                         "|CR|15Enter option number to |11MOVE|15 : ");
    value[PROMPT_MENU_OPTION_MOVE_TO] = std::make_pair("Menu Option To Copy To",
                                                       "|11SAVE|15 as and insert before which option : ");

    // Menu Option Edit Fields
    value[PROMPT_MENU_OPTION_EDIT_HEADER] = std::make_pair("Menu Option Fields Editor Header |OT Menu ID",
                                                           "|CS|CR|03--- |15[|03Oblivion/2 XRM |07// |11Menu Option Editor|15] |03--- |11Menu ID : |15|OT |CR");
    value[PROMPT_MENU_OPTION_FIELD_INPUT_TEXT] = std::make_pair("Menu Option Field Edit Prompt",
                                                                "|CR|15Option Editor C|07om|08mand |15: |07");

    // NOTE, added |PD will display the prompt description as HELP text to the user
    // Useful when editing fields - Specific to XRM.
    value[PROMPT_MENU_OPTION_FIELD_NAME] = std::make_pair("Name Displayed in an ANSI screen",
                                                          "|CR|03%   |15|PD|CR|11!   |03(|11A|03) |15Option Name        : ");
    value[PROMPT_MENU_OPTION_FIELD_ACS] = std::make_pair("Access Control String - Setting security and flags",
                                                         "|CR|03%   |15|PD|CR|11!   |03(|11B|03) |15ACS                : ");
    value[PROMPT_MENU_OPTION_FIELD_HIDDEN] = std::make_pair("Hidden Option, Exclude from ANSI screen",
                                                            "|CR|03%   |15|PD|CR|11!   |03(|11C|03) |15Hidden       |07(|15T|07/|15F|07)|15 : ");
    value[PROMPT_MENU_OPTION_FIELD_CMD_KEY] = std::make_pair(
        "Executes a specific menu function (Example '-/' to change menu)",
        "|CR|03%   |15|PD|CR|11!   |03(|11D|03) |15Command Keys       : ");
    value[PROMPT_MENU_OPTION_FIELD_MENU_KEY] = std::make_pair("Input keys user enters to execute menu option",
                                                              "|CR|03%   |15|PD|CR|11!   |03(|11E|03) |15Keys               : ");
    value[PROMPT_MENU_OPTION_FIELD_CMD_STRING] = std::make_pair(
        "Parameters required by Command Key (Example 'MAIN' for switching menus)",
        "|CR|03%   |15|PD|CR|11!   |03(|11F|03) |15Command String     : ");
    value[PROMPT_MENU_OPTION_FIELD_PULLDOWN] = std::make_pair(
        "Pulldown ID - the order light bars move from the beginning to the end",
        "|CR|03%   |15|PD|CR|11!   |03(|11G|03) |15Pulldown ID        : ");

    // Menu commands, some header info for the view toggle.
    value[PROMPT_OPTION_TOGGLE] = std::make_pair("Command View Toggle using 'T' cycles though different displays",
                                                 "|CR|03%   |15|PD|CR|11!   |15Current View : |11|OT |CR");
    ...
    value[DISPLAY_MENU_FIELDS_PULLDOWN_FILE] =
            std::make_pair("Pulldown File", " |03(|11F|03) |15Pulldown File      : ");
    value[DISPLAY_MENU_FIELDS_VIEW_GENERIC] = std::make_pair("View Generic Menu",
                                                             " |03(|11G|03) |15View Generic Menu    ");
    ...
    value[DISPLAY_MENU_FIELDS_QUIT_ABORT] = std::make_pair("Quit without Save",
    ...
    value[DISPLAY_OPT_FIELDS_OPTION_ACS] = std::make_pair("Access Control String",
                                                          " |03(|11B|03) |15ACS                : ");
    value[DISPLAY_OPT_FIELDS_OPTION_HIDDEN] = std::make_pair("Hidden", " |03(|11C|03) |15Hidden             : ");
    value[DISPLAY_OPT_FIELDS_OPTION_CMD_KEYS] =
            std::make_pair("Command Keys", " |03(|11D|03) |15Command Keys       : ");
    value[DISPLAY_OPT_FIELDS_OPTION_MENU_KEY] = std::make_pair("Menu Key", " |03(|11E|03) |15Keys               : ");
    value[DISPLAY_OPT_FIELDS_OPTION_CMD_STRING] = std::make_pair("Command String",
                                                                 " |03(|11F|03) |15Command String     : ");
    value[DISPLAY_OPT_FIELDS_OPTION_PULLDOWN_ID] = std::make_pair("Pulldown ID",
                                                                  " |03(|11G|03) |15Pulldown ID        : ");
    value[DISPLAY_OPT_FIELDS_OPTION_PREVIOUS_OPT] = std::make_pair("Previous Option",
                                                                   " |03(|11[|03) |15Previous Option      ");
...
void ModMenuEditor::setupMenuEditor() {
    ...
    m_menu_display_list = m_ctx.getIoCommon().splitString(menu_display_output, '\n');
    m_page = 0;
...
void ModMenuEditor::displayCurrentPage(const std::string &input_state) {
    ...
    int rows_used = m_ctx.getScreenAnsi().getMaxRowsUsedOnScreen();
    const int max_rows = m_ctx.getScreenAnsi().getMaxLines();
    ...
    for (int i = (m_page * (m_rows_per_page - 2)); i < m_menu_display_list.size(); i++) {
        std::string display_line = m_ctx.getIoSession().pipe2ansi(m_menu_display_list[i]);
        display_line.append("\r\n");
...
void ModMenuEditor::displayCurrentEditPage(const std::string &input_state) {
    for (int i = 0; i < m_menu_display_list.size(); i++) {
        std::string display_line = m_ctx.getIoSession().pipe2ansi(m_menu_display_list[i]);
        display_line.append("\r\n");
        baseProcessAndDeliver(display_line);
    }
 ...
 * Passthrough Input, Any Key hit to continue.
 */
...
void ModMenuEditor::menuEditorInput(const std::string &input) {
    ...
    // ESC was hit
    if (result.empty() || result == "aborted") {
        return;
    }
    ...
    if (result[0] == '\n') {
        ...
        switch (toupper(key[0])) {
            case 'A': // Menu Title
                ...
                m_ctx.getIoSession().getInputField("", key, Config::sName_length, m_loaded_menu.back().menu_title);
                break;
            ...
            case 'B': // Menu Password
                ...
                m_ctx.getIoSession().getInputField("", key, Config::sName_length, m_loaded_menu.back().menu_password);
                break;
            ...
            case 'C': // Menu Password
                ...
                m_ctx.getIoSession().getInputField("", key, Config::sName_length, m_loaded_menu.back().menu_fall_back);
                break;
            ...
            case 'D': // Menu Help ID
                ...
                m_ctx.getIoSession().getInputField("", key, Config::sName_length, m_loaded_menu.back().menu_help_file);
                break;
            ...
            case 'E': // Menu Name
                ...
                m_ctx.getIoSession().getInputField("", key, Config::sName_length, m_loaded_menu.back().menu_name);
                break;
            ...
            case 'F': // Menu Pulldown file
                ...
                m_ctx.getIoSession().getInputField("", key, Config::sName_length,
                                                   m_loaded_menu.back().menu_pulldown_file);
                break;
    ...
    if (result[0] == '\n') {
        ...
        switch (toupper(key[0])) {
            ...
            case 'C': // Option Hidden
            {
                ...
                bool_value += m_ctx.getIoCommon().boolAlpha(m_loaded_menu.back().menu_options[m_current_option].hidden).
                        at(0);
            ...
            case 'G': // Option Pulldown ID
                ...
                m_ctx.getIoSession().getInputField("", key, Config::sName_length,
                                                   std::to_string(
                                                       m_loaded_menu.back().menu_options[m_current_option].
                                                       pulldown_id));
                break;
    ...
    if (result[0] == '\n') {
        ...
        switch (m_current_field) {
            ...
            case 'C': // Menu Fallback
            {
                ...
                m_loaded_menu.back().menu_options[m_current_option].hidden = fallBackResult;
                break;
            }
...
void ModMenuEditor::deleteExistingMenuOption(unsigned int option_index) {
    int option_size = m_loaded_menu.back().menu_options.size();
    ...
    for (int i = 0; i < option_size; i++) {
        ...
        if (m.index == option_index) {
            ...
            const MenuOption temp = m_loaded_menu.back().menu_options[i];
...
void ModMenuEditor::reorderMenuIndexesInsertion(unsigned int option_index) {
    for (int i = 0; i < m_loaded_menu.back().menu_options.size(); i++) {
        auto &m = m_loaded_menu.back().menu_options[i];
...
std::string ModMenuEditor::displayMenuOptionList() {
    Menu current_menu = m_loaded_menu.back();
    ...
    for (unsigned int i = 0; i < current_menu.menu_options.size(); i++) {
        ...
        std::string option_string = m_ctx.getIoCommon().rightPadding(std::to_string(m.index), 3);

        // Toggled View will show commands strings, while default shows command key info.
        switch (m_mod_toggle_view_index) {
            case VIEW_DEFAULT:
                option_string.append(baseGetDefaultColor() + m_ctx.getIoCommon().rightPadding(m.menu_key, 9));
                option_string.append(baseGetDefaultInputColor() + m_ctx.getIoCommon().rightPadding(m.command_key, 4));
                option_string.append(baseGetDefaultStatColor() + m_ctx.getIoCommon().rightPadding(m.acs_string, 8));
                break;

            case VIEW_NAMES:
                option_string.append(baseGetDefaultColor() + m_ctx.getIoCommon().rightPadding(m.name, 21));
                break;

            case VIEW_STRINGS:
                option_string.append(baseGetDefaultColor() + m_ctx.getIoCommon().rightPadding(m.command_string, 15));
                option_string.append(baseGetDefaultInputColor() + m_ctx.getIoCommon().rightPadding("", 1));
                option_string.append(
                    baseGetDefaultStatColor() + m_ctx.getIoCommon().rightPadding(
                        m_ctx.getIoCommon().boolAlpha(m.hidden), 5));
                break;
            ...
            case VIEW_PULLDOWN:
                ...
                if (m.pulldown_id > 0) {
                    option_string.append(
                        baseGetDefaultStatColor() + m_ctx.getIoCommon().rightPadding(std::to_string(m.pulldown_id), 8));
                } else {
                    option_string.append(m_ctx.getIoCommon().rightPadding("", 8));
                }
            default: break;
        }
...
std::string ModMenuEditor::displayMenuEditScreen() {
    ...
    result_set.push_back(
        getDisplayPromptRaw(DISPLAY_MENU_FIELDS_VERSION_ID) + baseGetDefaultStatColor() + m_ctx.getIoCommon().
        rightPadding(current_menu.file_version, 48));
    result_set.push_back(baseGetDefaultPromptColor() + " " + encode.utf8Encode(std::string(72, M_BORDER_ROW)) + " ");
    result_set.push_back(
        getDisplayPromptRaw(DISPLAY_MENU_FIELDS_TITLE) + baseGetDefaultStatColor() + m_ctx.getIoCommon().rightPadding(
            current_menu.menu_title, 48));
    result_set.push_back(
        getDisplayPromptRaw(DISPLAY_MENU_FIELDS_PASSWORD) + baseGetDefaultStatColor() + m_ctx.getIoCommon().
        rightPadding(current_menu.menu_password, 48));
    result_set.push_back(
        getDisplayPromptRaw(DISPLAY_MENU_FIELDS_FALLBACK) + baseGetDefaultStatColor() + m_ctx.getIoCommon().
        rightPadding(current_menu.menu_fall_back, 48));
    result_set.push_back(
        getDisplayPromptRaw(DISPLAY_MENU_FIELDS_HELP_ID) + baseGetDefaultStatColor() + m_ctx.getIoCommon().rightPadding(
            current_menu.menu_help_file, 48));
    result_set.push_back(
        getDisplayPromptRaw(DISPLAY_MENU_FIELDS_NAME) + baseGetDefaultStatColor() + m_ctx.getIoCommon().rightPadding(
            current_menu.menu_name, 48));
    result_set.push_back(
        getDisplayPromptRaw(DISPLAY_MENU_FIELDS_PULLDOWN_FILE) + baseGetDefaultStatColor() + m_ctx.getIoCommon().
        rightPadding(current_menu.menu_pulldown_file, 48));
    result_set.push_back(
        getDisplayPromptRaw(DISPLAY_MENU_FIELDS_VIEW_GENERIC) + baseGetDefaultStatColor() + m_ctx.getIoCommon().
        rightPadding("", 48));
    result_set.push_back(
        getDisplayPromptRaw(DISPLAY_MENU_FIELDS_EDIT_OPTIONS) + baseGetDefaultStatColor() + m_ctx.getIoCommon().
        rightPadding("", 48));
    result_set.push_back(baseGetDefaultPromptColor() + " " + encode.utf8Encode(std::string(72, M_BORDER_ROW)) + " ");
    result_set.push_back(
        getDisplayPromptRaw(DISPLAY_MENU_FIELDS_QUIT_SAVE) + baseGetDefaultStatColor() + m_ctx.getIoCommon().
        rightPadding("", 48));
    result_set.push_back(
        getDisplayPromptRaw(DISPLAY_MENU_FIELDS_QUIT_ABORT) + baseGetDefaultStatColor() + m_ctx.getIoCommon().
        rightPadding("", 48));

    // Not in use Yet, seems legacy only does ACS in option commands.
    // option_string.append("Menu ACS           : " + m_ctx.getIoCommon().rightPadding(current_menu.menu_acs_string, 35);
    // option_string.append("Menu FormMenu      : " + m_ctx.getIoCommon().rightPadding(current_menu.menu_form_menu, 8);
...
std::string ModMenuEditor::displayMenuOptionEditScreen() {
    ...
    std::string option_string = std::to_string(opt.index) + " / " + std::to_string(
                                    m_loaded_menu.back().menu_options.size() - 1);

    const IoEncoding encode;
    result_set.push_back(
        getDisplayPromptRaw(DISPLAY_OPT_FIELDS_OPTION_ID) + baseGetDefaultStatColor() + m_ctx.getIoCommon().
        rightPadding(option_string, 48));
    result_set.push_back(baseGetDefaultPromptColor() + " " + encode.utf8Encode(std::string(72, M_BORDER_ROW)) + " ");
    result_set.push_back(
        getDisplayPromptRaw(DISPLAY_OPT_FIELDS_OPTION_NAME) + baseGetDefaultStatColor() + m_ctx.getIoCommon().
        rightPadding(opt.name, 48));
    result_set.push_back(
        getDisplayPromptRaw(DISPLAY_OPT_FIELDS_OPTION_ACS) + baseGetDefaultStatColor() + m_ctx.getIoCommon().
        rightPadding(opt.acs_string, 48));
    result_set.push_back(
        getDisplayPromptRaw(DISPLAY_OPT_FIELDS_OPTION_HIDDEN) + baseGetDefaultStatColor() + m_ctx.getIoCommon().
        rightPadding(m_ctx.getIoCommon().boolAlpha(opt.hidden), 48));
    result_set.push_back(
        getDisplayPromptRaw(DISPLAY_OPT_FIELDS_OPTION_CMD_KEYS) + baseGetDefaultStatColor() + m_ctx.getIoCommon().
        rightPadding(opt.command_key, 48));
    result_set.push_back(
        getDisplayPromptRaw(DISPLAY_OPT_FIELDS_OPTION_MENU_KEY) + baseGetDefaultStatColor() + m_ctx.getIoCommon().
        rightPadding(opt.menu_key, 48));
    result_set.push_back(
        getDisplayPromptRaw(DISPLAY_OPT_FIELDS_OPTION_CMD_STRING) + baseGetDefaultStatColor() + m_ctx.getIoCommon().
        rightPadding(opt.command_string, 48));
    result_set.push_back(
        getDisplayPromptRaw(DISPLAY_OPT_FIELDS_OPTION_PULLDOWN_ID) + baseGetDefaultStatColor() + m_ctx.getIoCommon().
        rightPadding(std::to_string(opt.pulldown_id), 48));
    result_set.push_back(
        getDisplayPromptRaw(DISPLAY_OPT_FIELDS_OPTION_PREVIOUS_OPT) + baseGetDefaultStatColor() + m_ctx.getIoCommon().
        rightPadding("", 48));
    result_set.push_back(
        getDisplayPromptRaw(DISPLAY_OPT_FIELDS_OPTION_NEXT_OPT) + baseGetDefaultStatColor() + m_ctx.getIoCommon().
        rightPadding("", 48));
    result_set.push_back(baseGetDefaultPromptColor() + " " + encode.utf8Encode(std::string(72, M_BORDER_ROW)) + " ");
    result_set.push_back(
        getDisplayPromptRaw(DISPLAY_OPT_FIELDS_OPTION_QUIT) + baseGetDefaultStatColor() + m_ctx.getIoCommon().
        rightPadding("", 48));

    // Not in use Yet, seems legacy only does ACS in option commands.
    // option_string.append("Form Value         : " + m_ctx.getIoCommon().rightPadding(opt.form_value, 48));

    // iterate through and print out
    int total_rows = result_set.size();
    total_rows += 2;

    // Could re-calc this on-screen width later on.
    int max_cols = 76;
