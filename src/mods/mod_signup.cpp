...
ModSignup::ModSignup(Context &ctx)
    ...
    m_setup_functions.emplace_back([this] { setupNewUserPassword(); });
    m_setup_functions.emplace_back([this] { setupDisclaimer(); });
    m_setup_functions.emplace_back([this] { setupHandle(); });
    m_setup_functions.emplace_back([this] { setupRealName(); });
    m_setup_functions.emplace_back([this] { setupAddress(); });
    m_setup_functions.emplace_back([this] { setupLocation(); });
    m_setup_functions.emplace_back([this] { setupCountry(); });
    m_setup_functions.emplace_back([this] { setupEmail(); });
    m_setup_functions.emplace_back([this] { setupUserNote(); });
    m_setup_functions.emplace_back([this] { setupBirthday(); });
    m_setup_functions.emplace_back([this] { setupGender(); });

    m_setup_functions.emplace_back([this] { setupPassword(); });
    m_setup_functions.emplace_back([this] { setupVerifyPassword(); });
    m_setup_functions.emplace_back([this] { setupChallengeQuestion(); });
    m_setup_functions.emplace_back([this] { setupChallengeAnswer(); });
    m_setup_functions.emplace_back([this] { setupVerifyChallengeAnswer(); });

    m_setup_functions.emplace_back([this] { setupYesNoBars(); });
    m_setup_functions.emplace_back([this] { setupDoPause(); });
    m_setup_functions.emplace_back([this] { setupClearOrScroll(); });
    m_setup_functions.emplace_back([this] { setupAnsiColor(); });
    m_setup_functions.emplace_back([this] { setupBackSpace(); });
    m_setup_functions.emplace_back([this] { setupVerifyAndSave(); });

    // Input Processing Functions
    m_mod_functions.emplace_back(
        [this](const std::string &s) { return newUserPassword(s); });
    m_mod_functions.emplace_back(
        [this](const std::string &s) { return disclaimer(s); });
    m_mod_functions.emplace_back(
        [this](const std::string &s) { return handle(s); });
    m_mod_functions.emplace_back(
        [this](const std::string &s) { return realName(s); });
    m_mod_functions.emplace_back(
        [this](const std::string &s) { return address(s); });
    m_mod_functions.emplace_back(
        [this](const std::string &s) { return location(s); });
    m_mod_functions.emplace_back(
        [this](const std::string &s) { return country(s); });
    m_mod_functions.emplace_back(
        [this](const std::string &s) { return email(s); });
    m_mod_functions.emplace_back(
        [this](const std::string &s) { return userNote(s); });
    m_mod_functions.emplace_back(
        [this](const std::string &s) { return birthday(s); });
    m_mod_functions.emplace_back(
        [this](const std::string &s) { return gender(s); });

    m_mod_functions.emplace_back(
        [this](const std::string &s) { return password(s); });
    m_mod_functions.emplace_back(
        [this](const std::string &s) { return verifyPassword(s); });
    m_mod_functions.emplace_back(
        [this](const std::string &s) { return challengeQuestion(s); });
    m_mod_functions.emplace_back(
        [this](const std::string &s) { return challengeAnswer(s); });
    m_mod_functions.emplace_back(
        [this](const std::string &s) { return verifyChallengeAnswer(s); });

    m_mod_functions.emplace_back(
        [this](const std::string &s) { return yesNoBars(s); });
    m_mod_functions.emplace_back(
        [this](const std::string &s) { return doPause(s); });
    m_mod_functions.emplace_back(
        [this](const std::string &s) { return clearOrScroll(s); });
    m_mod_functions.emplace_back(
        [this](const std::string &s) { return ansiColor(s); });
    m_mod_functions.emplace_back(
        [this](const std::string &s) { return backSpace(s); });
    m_mod_functions.emplace_back(
        [this](const std::string &s) { return verifyAndSave(s); });

    // Check if the Text Prompts exist.
...
void ModSignup::createTextPrompts() {
    ...
    value[PROMPT_CHALLENGE_ANSWER] = std::make_pair("Forgot Password Challenge Answer",
                                                    "(case sensitive) Challenge Answer: ");
    ...
    value[PROMPT_USE_CLEAR] = std::make_pair("Clear Screen or Scroll",
                                             "[y/ENTER or n] Use Clear Screen or Scrolling: ");
    value[PROMPT_USE_ANSI_COLOR] = std::make_pair("Use Ansi Colors", "[y/ENTER or n] Use Ansi Colors: ");
    value[PROMPT_BACK_SPACE] = std::make_pair("Backspace Sequence",
                                              "[W]indows/[T]erminal/[ENTER] to Set Backspace Key: ");
    ...
    value[PROMPT_PASS_INVALID] = std::make_pair("Invalid/Non Matching Password",
                                                "|04Invalid, password does not match!|CR");
    value[PROMPT_HANDLE_INVALID] =
            std::make_pair("User Name Already Exists", "|04Invalid UserName, Already Exists!|CR");
    value[PROMPT_NAME_INVALID] = std::make_pair("Real Name Already Exists",
                                                "|04Name, Already Exists, Try Adding a middle initial.|CR");
    value[PROMPT_EMAIL_INVALID] = std::make_pair("Email Already Exists",
                                                 "|04Email, Already Exists, Try another address or check if your have an account.|CR");
    ...
    // Auto Validate User
    value[PROMPT_AUTO_VALIDATION] = std::make_pair("User Access Auto Validated",
                                                   "|10Auto Validation, |15Access has been granted to the system.|CR");
    value[PROMPT_NOT_VALIDATED] = std::make_pair("Access Restricted, Waiting for Validation",
                                                 "|04Access Restricted, |15Waiting for Access Validation.|CR");

    // New User Voting
    value[PROMPT_NEWUSER_VOTING] = std::make_pair("New User Voting",
                                                  "|04New User Voting Active, |15Access will be granted once your account has been voted on.|CR");
...
void ModSignup::setupNewUserPassword() {
    ...
    if (m_ctx.getCfg().use_newuser_password) {
        // Kick back to Matrix if we exceed the number of attempts.
        if (m_newuser_password_attempts >= m_ctx.getCfg().invalid_newuser_password_attempts) {
            m_is_active = false;
            return;
        }
...
void ModSignup::setupDisclaimer() {
    if (m_ctx.getCfg().use_disclaimer) {
...
void ModSignup::setupChallengeQuestion() {
    if (m_ctx.getCfg().use_challenge_question) {
...
void ModSignup::setupYesNoBars() {
    if (m_ctx.getCfg().use_yesno_bars) {
...
void ModSignup::setupClearOrScroll() {
    if (m_ctx.getCfg().use_clear_screen) {
...
void ModSignup::setupAnsiColor() {
    if (m_ctx.getCfg().use_ansi_color) {
...
bool ModSignup::newUserPassword(const std::string &input) {
    ...
    (std::string &input, std::string &key) {
        // Debug Logging
        m_log.log(Logging::LogLevel::Debug, "Current Input", input, __LINE__, __FILE__);

        if (key.compare(m_ctx.getCfg().password_newuser) == 0) {
            m_log.log(Logging::LogLevel::Console, "NewUserPassword Accepted", __LINE__, __FILE__);
            changeNextModule();
        } else {
            m_log.log(Logging::LogLevel::Console, "Error, Incorrect NewUserPassword!", __LINE__, __FILE__);
...
bool ModSignup::handle(const std::string &input) {
    ...
    (std::string &input, std::string &key) {
        ...
        Users search = user_data.getUserByHandle(key);

        if (search.iId == -1) {
            m_new_use_rec.sHandle = key;
            changeNextModule();
        } else {
...
bool ModSignup::realName(const std::string &input) {
    ...
    (std::string &input, std::string &key) {
        ...
        UsersDao user_data(m_ctx.getDatabase());
        Users search = user_data.getUserByRealName(key);

        if (search.iId == -1) {
            m_new_use_rec.sRealName = key;
            changeNextModule();
        } else {
...
bool ModSignup::email(const std::string &input) {
    ...
    (std::string &input, std::string &key) {
        ...
        UsersDao user_data(m_ctx.getDatabase());
        Users search = user_data.getUserByEmail(key);

        if (search.iId == -1) {
            m_new_use_rec.sEmail = key;
            changeNextModule();
        } else {
...
bool ModSignup::birthday(const std::string &input) {
    ...
    (std::string &input, std::string &key) {
        ...
        try {
            ...
            m_log.log(Logging::LogLevel::Error, "Error, regex date error=", ex.what(), ex.code(), __LINE__, __FILE__);
        }
...
bool ModSignup::verifyPassword(const std::string &input) {
    ...
    (std::string &input, std::string &key) {
        ...
        if (m_security_record.sPasswordHash.compare(key) == 0) {
            ...
            Encrypt encryption;
            std::string salt = encryption.generate_salt(m_new_use_rec.sHandle, m_ctx.getCfg().bbs_uuid);
            ...
            m_log.log(Logging::LogLevel::Info, "Password Verify Failed.", __LINE__, __FILE__);
    ...
    (std::string &input, std::string &key) {
        ...
        if (m_security_record.sChallengeAnswerHash.compare(key) == 0) {
            ...
            if (password.size() == 0) {
                ...
                m_log.log(Logging::LogLevel::Error, "Error, ChallengeAnswer was empty", __LINE__, __FILE__);
                return false;
            }
            ...
            m_log.log(Logging::LogLevel::Error, "Error, ChallengeAnswer verify failed.", __LINE__, __FILE__);
            m_security_record.sChallengeAnswerHash = "";
            displayPromptAndNewLine(PROMPT_PASS_INVALID);
            changePreviousModule();
        }
...
bool ModSignup::yesNoBars(const std::string &input) {
    ...
    if (result == "aborted") {
        ...
        if (key.size() == 0 || (toupper(key[0]) == 'Y' && key.size() == 1)) {
            ...
            m_new_use_rec.bYesNoBars = true;
            baseProcessDeliverNewLine();
        }
        // Else check for single N for No to default to ASCII no colors.
        else if (toupper(key[0]) == 'N' && key.size() == 1) {
            m_new_use_rec.bYesNoBars = false;
            baseProcessDeliverNewLine();
        } else {
            baseProcessDeliverNewLine();
            displayPromptAndNewLine(PROMPT_TEXT_INVALID);
...
bool ModSignup::doPause(const std::string &input) {
    ...
    if (result == "aborted") {
        ...
        if (key.size() == 0 || (toupper(key[0]) == 'Y' && key.size() == 1)) {
            ...
            m_new_use_rec.bDoPause = true;
            baseProcessDeliverNewLine();
        }
        // Else check for single N for No to default to ASCII no colors.
        else if (toupper(key[0]) == 'N' && key.size() == 1) {
            m_new_use_rec.bDoPause = false;
            baseProcessDeliverNewLine();
        } else {
            baseProcessDeliverNewLine();
            displayPromptAndNewLine(PROMPT_TEXT_INVALID);
...
bool ModSignup::clearOrScroll(const std::string &input) {
    ...
    if (result == "aborted") {
        ...
        if (key.size() == 0 || (toupper(key[0]) == 'Y' && key.size() == 1)) {
            ...
            m_new_use_rec.bClearOrScroll = true;
            baseProcessDeliverNewLine();
        }
        // Else check for single N for No to default to ASCII no colors.
        else if (toupper(key[0]) == 'N' && key.size() == 1) {
            m_new_use_rec.bClearOrScroll = false;
            baseProcessDeliverNewLine();
        } else {
            baseProcessDeliverNewLine();
            displayPromptAndNewLine(PROMPT_TEXT_INVALID);
...
bool ModSignup::ansiColor(const std::string &input) {
    ...
    if (result == "aborted") {
        ...
        if (key.size() == 0 || (toupper(key[0]) == 'Y' && key.size() == 1)) {
            ...
            m_new_use_rec.bAnsi = true;
            baseProcessDeliverNewLine();
        }
        // Else check for single N for No to default to ASCII no colors.
        else if (toupper(key[0]) == 'N' && key.size() == 1) {
            m_new_use_rec.bAnsi = false;
            baseProcessDeliverNewLine();
        } else {
            baseProcessDeliverNewLine();
            displayPromptAndNewLine(PROMPT_TEXT_INVALID);
...
bool ModSignup::backSpace(const std::string &input) {
    ...
    if (result == "aborted") {
        ...
        // Else check for single N for No to default to ASCII no colors.
            ...
            m_new_use_rec.bBackSpaceVt100 = true;
            baseProcessDeliverNewLine();
        } else {
            baseProcessDeliverNewLine();
            displayPromptAndNewLine(PROMPT_TEXT_INVALID);
...
void ModSignup::saveNewUserRecord() {
    ...
    UsersDao user_dao(m_ctx.getDatabase());
    ...
    m_new_use_rec.iSecurityIndex = securityIndex;

    //time_t const t = (time_t)time(0);
    std::time_t tt = 0;
    std::time_t const result = std::time(&tt);

    m_new_use_rec.dtFirstOn = result;
    m_new_use_rec.dtPassChangeDate = result;

    // Set Default User Theme colors, pull from system defaults.
    m_new_use_rec.sRegColor = m_ctx.getCfg().default_color_regular;
    m_new_use_rec.sPromptColor = m_ctx.getCfg().default_color_prompt;
    m_new_use_rec.sInputColor = m_ctx.getCfg().default_color_input;
    m_new_use_rec.sInverseColor = m_ctx.getCfg().default_color_inverse;
    m_new_use_rec.sStatColor = m_ctx.getCfg().default_color_stat;
    m_new_use_rec.sBoxColor = m_ctx.getCfg().default_color_box;

    // Check for User Auto Validation
    if (m_ctx.getCfg().use_auto_validate_users) {
        m_new_use_rec.iLevel = m_ctx.getCfg().default_level;
        m_new_use_rec.iFileLevel = m_ctx.getCfg().default_file_level;
        m_new_use_rec.iMessageLevel = m_ctx.getCfg().default_message_level;
        m_new_use_rec.iFilePoints = m_ctx.getCfg().default_file_points;

        // Also Add Default File points,, missing from user rec.
    }

    long userIndex = user_dao.insertRecord(m_new_use_rec);
    m_log.log(Logging::LogLevel::Info, "New User Index saved", userIndex);
    ...
    if (userIndex < 0) {
        ...
        if (!security_dao.deleteRecord(securityIndex)) {
            m_log.log(Logging::LogLevel::Error, "Error, unable to remove security record.", __LINE__, __FILE__);
        }
    ...
    if (userIndex == 1) {
        m_log.log(Logging::LogLevel::Info, "Updating First Created User to SysOp/Admin");

        // Re-Read the Record Clean
        m_new_use_rec = user_dao.getRecordById(userIndex);
        m_new_use_rec.iLevel = 255;
        m_new_use_rec.iFileLevel = 255;
        m_new_use_rec.iMessageLevel = 255;

        // Save Updates for Admin Access
        user_dao.updateRecord(m_new_use_rec);
    }
