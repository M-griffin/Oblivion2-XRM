...
Encoding::CharSet Encoding::detectEncoding(const std::string &data) {
    if (isValidUtf8(data))
        return CharSet::UTF8;
...
std::string Encoding::cp437ToUtf8(const std::string &input) const {
...
std::string Encoding::utf8ToCp437(const std::string &input) const {
    return preserveAnsiConvert(input, [this](const std::string &s) {
        std::string out;
        try {
            ...
            for (; it != end; ++it) {
                ...
                if (found != unicodeToCp437.end())
                    ...
                    out.push_back('?');
                    m_log.write<Logging::DEBUG_LOG>("Unmapped Unicode in CP437", cp);
                }
            }
        } catch (...) { m_log.write<Logging::ERROR_LOG>("UTF8->CP437 conversion failed"); }
        return out;
    });
}

std::string Encoding::iso8859_1ToUtf8(const std::string &input) const {
...
std::string Encoding::utf8ToIso8859_1(const std::string &input) const {
    return preserveAnsiConvert(input, [](const std::string &s) {
        std::string out;
        try {
            utf8::unchecked::iterator<std::string::const_iterator> it(s.begin()), end(s.end());
...
std::string Encoding::utf8Encode(const std::string &s) const {
    return cp437ToUtf8(s);
}

std::string Encoding::utf8Decode(const std::string &s) const {
