...
Users UsersDao::getUserByHandle(std::string name) {
    ...
    if (!m_database.isConnected()) {
        m_log.log(UtilLog::LogLevel::Error, "Error, Database is not connected!", m_strTableName, __LINE__, __FILE__);
        return user;
    }
    ...
    if (!qry.isConnected()) {
        m_log.log(UtilLog::LogLevel::Error, "Error, Query has no connection to the database", m_strTableName, __LINE__,
                                        __FILE__);
        return user;
    }
    ...
    if (qry.getResult(queryString)) {
        ...
        if (rows > 0) {
            ...
            m_log.log(UtilLog::LogLevel::Info, "NotFound, getUserByHandle Returned Rows=", rows, m_strTableName, __LINE__,
                                           __FILE__);
        }
    } else {
        m_log.log(UtilLog::LogLevel::Error, "Error, getResult()", m_strTableName, __LINE__, __FILE__);
    }
...
Users UsersDao::getUserByRealName(std::string name) {
    ...
    if (qry.getResult(queryString)) {
        ...
        if (rows > 0) {
            ...
            m_log.log(UtilLog::LogLevel::Info, "Notfound, getUserByRealName Returned Rows", rows, m_strTableName, __LINE__,
                                           __FILE__);
        }
    } else {
        m_log.log(UtilLog::LogLevel::Error, "Error, getResult()", m_strTableName, __LINE__, __FILE__);
...
Users UsersDao::getUserByEmail(std::string email) {
    ...
    if (qry.getResult(queryString)) {
        ...
        if (rows > 0) {
            ...
            m_log.log(UtilLog::LogLevel::Info, "Notfound, getUserByEmail Returned Rows=", rows, m_strTableName, __LINE__,
                                           __FILE__);
        }
    } else {
        m_log.log(UtilLog::LogLevel::Error, "Error, getResult()", m_strTableName, __LINE__, __FILE__);
...
std::vector<Users> UsersDao::getUsersByWildcard(std::string filter) {
    ...
    if (qry.getResult(queryString)) {
        ...
        if (rows > 0) {
            ...
            m_log.log(UtilLog::LogLevel::Info, "Notfound, getUsersByWildcard Returned Rows=", rows, m_strTableName,
                                           __LINE__, __FILE__);
        }
    } else {
        m_log.log(UtilLog::LogLevel::Error, "Error, getResult()", m_strTableName, __LINE__, __FILE__);
