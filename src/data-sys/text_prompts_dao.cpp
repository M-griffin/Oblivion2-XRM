...
bool TextPromptsDao::readPrompts() {
    ...
    try {
        ...
        const auto file_version = node["file_version"].as<std::string>();
        ...
        if (file_version != TextPromptsDao::FILE_VERSION) {
            m_log.write<Logging::ERROR_LOG>("Text Prompt Invalid File Version=", file_version, "Expected=",
 ...
 * @return
 */
M_StringPair TextPromptsDao::getPrompt(const std::string &lookup) const {
    if (!m_is_loaded || (TEXT_PROMPTS[m_filename].empty())) {
        M_StringPair temp;
 ...
 */
void TextPromptsDao::displayAll() const {
 ...
 */
void TextPromptsDao::cacheAllTextPrompts(YAML::Node &node) const {
    try {
        ...
        for (YAML::const_iterator it = node.begin(); it != node.end(); ++it) {
            ...
            // Exclude This when preloading all prompts.
            if (it->first.as<std::string>() != "file_version") {
                auto key = it->first.as<std::string>();
                const auto value = it->second.as<M_StringPair>();
