...
ModLevelEditor::ModLevelEditor(session_ptr session_data, config_ptr config, processor_ansi_ptr ansi_process,
    ...
    : ModBase(session_data, config, ansi_process, "mod_level_editor.yaml", common_io, session_io)
...
void ModLevelEditor::setupLevelEditor()
{
    ...
    m_level_display_list = m_io_common.splitString(level_display_output, '\n');
    m_page = 0;
    ...
    else if(result[0] == '\n')
    {
        ...
        switch(toupper(key[0]))
        {
            ...
            case 'M': // Bool PostCallRatio
            {
                ...
                bool_value = m_io_common.boolAlpha(current_level->bPostCallRatio).at(0);
...
void ModLevelEditor::levelEditorLevelFieldHandler(const std::string& input)
{
    ...
    else if(result[0] == '\n')
    {
        ...
        switch(m_current_field)
        {
            ...
            case 'C': // Level File Level
                current_level->iFileLevel = m_io_common.stringToInt(key);
                break;

            case 'D': // Level Message Level
                current_level->iMessageLevel = m_io_common.stringToInt(key);
                break;

            case 'E': // Level Post/Call Ratio
                current_level->iPostCallRatio = m_io_common.stringToInt(key);
                break;

            case 'F': // Level File Ratio
                current_level->iFileRatio = m_io_common.stringToInt(key);
                break;

            case 'G': // Level Time Limit
                current_level->iTimeLimit = m_io_common.stringToInt(key);
                break;

            case 'H': // Level Call Limit
                current_level->iCallLimit = m_io_common.stringToInt(key);
                break;

            case 'I': // Level Downloads
                current_level->iDownloads = m_io_common.stringToInt(key);
                break;

            case 'J': // Level Downloads MB
                current_level->iDownloadMB = m_io_common.stringToInt(key);
                break;
            ...
            case 'M': // Bool PostCallRatio
                current_level->bPostCallRatio = m_io_common.stringToBool(key);
                break;

            case 'N': // Bool File Ratio
                current_level->bFileRatio = m_io_common.stringToBool(key);
                break;

            case 'O': // Bool Time Limit
                current_level->bTimeLimit = m_io_common.stringToBool(key);
                break;

            case 'P': // Bool Call Limit
                current_level->bCallLimit = m_io_common.stringToBool(key);
                break;

            case 'R': // Bool Download File Limit
                current_level->bDownloads = m_io_common.stringToBool(key);
                break;

            case 'S': // Bool Download Limit MB
                current_level->bDownloadMB = m_io_common.stringToBool(key);
                break;
        }
...
std::string ModLevelEditor::displayLevelList()
{
    ...
    for(unsigned int i = 0; i < m_loaded_levels.size(); i++)
    {
        std::string option_string = m_io_common.rightPadding(std::to_string(m_loaded_levels[i]->iLevel), 5);
        option_string.append(baseGetDefaultStatColor() + m_io_common.rightPadding(m_loaded_levels[i]->sName, 19));
...
std::string ModLevelEditor::displayLevelEditScreen()
{
    ...
    result_set.push_back(getDisplayPromptRaw(DISPLAY_LEVEL_FIELD_NAME) + baseGetDefaultStatColor() +
                         m_io_common.rightPadding(current_level->sName, 48));

    result_set.push_back(getDisplayPromptRaw(DISPLAY_LEVEL_FIELD_START_MENU) + baseGetDefaultStatColor() +
                         m_io_common.rightPadding(current_level->sStartMenu, 48));


    result_set.push_back(getDisplayPromptRaw(DISPLAY_LEVEL_FIELD_FILE_LEVEL) + baseGetDefaultStatColor() +
                         m_io_common.rightPadding(std::to_string(current_level->iFileLevel), 48));

    result_set.push_back(getDisplayPromptRaw(DISPLAY_LEVEL_FIELD_MESG_LEVEL) + baseGetDefaultStatColor() +
                         m_io_common.rightPadding(std::to_string(current_level->iMessageLevel), 48));

    result_set.push_back(getDisplayPromptRaw(DISPLAY_LEVEL_FIELD_POST_CALL_RATIO) + baseGetDefaultStatColor() +
                         m_io_common.rightPadding(std::to_string(current_level->iPostCallRatio), 48));

    result_set.push_back(getDisplayPromptRaw(DISPLAY_LEVEL_FIELD_FILE_RATIO) + baseGetDefaultStatColor() +
                         m_io_common.rightPadding(std::to_string(current_level->iFileRatio), 48));

    result_set.push_back(getDisplayPromptRaw(DISPLAY_LEVEL_FIELD_TIME_LIMIT) + baseGetDefaultStatColor() +
                         m_io_common.rightPadding(std::to_string(current_level->iTimeLimit), 48));

    result_set.push_back(getDisplayPromptRaw(DISPLAY_LEVEL_FIELD_CALL_LIMIT) + baseGetDefaultStatColor() +
                         m_io_common.rightPadding(std::to_string(current_level->iCallLimit), 48));

    result_set.push_back(getDisplayPromptRaw(DISPLAY_LEVEL_FIELD_DOWNLOADS) + baseGetDefaultStatColor() +
                         m_io_common.rightPadding(std::to_string(current_level->iDownloads), 48));

    result_set.push_back(getDisplayPromptRaw(DISPLAY_LEVEL_FIELD_DOWNLOAD_MB) + baseGetDefaultStatColor() +
                         m_io_common.rightPadding(std::to_string(current_level->iDownloadMB), 48));

    result_set.push_back(getDisplayPromptRaw(DISPLAY_LEVEL_FIELD_ARFLAGS1) + baseGetDefaultStatColor() +
                         m_io_common.rightPadding(acs.getAccessConditionFlagStringFromBits(current_level->iARFlags1), 48));

    result_set.push_back(getDisplayPromptRaw(DISPLAY_LEVEL_FIELD_ARFLAGS2) + baseGetDefaultStatColor() +
                         m_io_common.rightPadding(acs.getAccessConditionFlagStringFromBits(current_level->iARFlags2), 48));


    result_set.push_back(m_io_common.rightPadding(getDisplayPromptRaw(DISPLAY_LEVEL_BOOL_POST_CALL_RATIO) + baseGetDefaultStatColor() + m_io_common.boolAlpha(current_level->bPostCallRatio), 56) +
                         m_io_common.rightPadding(getDisplayPromptRaw(DISPLAY_LEVEL_BOOL_CALL_LIMIT) + baseGetDefaultStatColor() + m_io_common.boolAlpha(current_level->bCallLimit), 56));


    result_set.push_back(m_io_common.rightPadding(getDisplayPromptRaw(DISPLAY_LEVEL_BOOL_FILE_RATIO) + baseGetDefaultStatColor() + m_io_common.boolAlpha(current_level->bFileRatio), 56) +
                         m_io_common.rightPadding(getDisplayPromptRaw(DISPLAY_LEVEL_BOOL_DOWNLOADS) + baseGetDefaultStatColor() + m_io_common.boolAlpha(current_level->bDownloads), 56));

    result_set.push_back(m_io_common.rightPadding(getDisplayPromptRaw(DISPLAY_LEVEL_BOOL_TIME_LIMIT) + baseGetDefaultStatColor() + m_io_common.boolAlpha(current_level->bTimeLimit), 56) +
                         m_io_common.rightPadding(getDisplayPromptRaw(DISPLAY_LEVEL_BOOL_DOWNLOAD_MB) + baseGetDefaultStatColor() + m_io_common.boolAlpha(current_level->bDownloadMB), 56));
